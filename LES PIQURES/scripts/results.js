...
function PageLoadResult()
{
	...
	else
	{
		...
		{
			PrintWarning("Vous devez repasser le quiz car la note est inférieure à " + options.upperMarkForRetake, 'R');
		}
...
function PrintReportQuestionDetails(qstNum,question)
{
	...
	if (question.valid === 1) {
		resultMsg = "Bonne réponse";
	}
	else if (question.valid === -1) {
		if (question.noChoice) {
			resultMsg = "Question sans réponse";
		}
		else {
			resultMsg = "Mauvaise réponse";
		}
	}
	else if ((question.valid === 2) && !options.reportHideRowQstNotValuated) {
		resultMsg = "Questions non évaluées";
	}
		...
		if (question.nScore === question.maxScore) {
			...
			resultMsg = "Réponse partiellement correcte";
		}
	}
	...
	//{
	//	textHtml += ". La réponse correcte est " + GetListOfRightAnsLetters(question);
	//}
	textHtml += "</i></p>";
	...
	if (question.valid === 3)
	{
		if (question.minScore === 0) {
			if (question.typeOfQuestion === QMAKE_MATCHING) {
				textHtml += "<p class='revision-class'><i>Phrases correctes : " + question.nScore + "/" + question.maxScore + "</i></p>";
			}
			else {
...
{
	PrintSuccess('Quiz enregistré correctement', 'R');
	deleteCookie("qmake.quiz");
}
...
function PostResultsFailCallback(jqXHR, exception)
{
	...
	PrintError('Impossible d&apos;enregistrer les résultats du quiz : ' + msg, 'R');
}
...
function PostLogin(email, passhash, domainId) {
	...
	$.ajax({
		...
		error: function (xhr, textStatus, errorThrown) {
			if (xhr.status === 401) {
				PrintError('Utilisateur non autorisé', 'R');
			}
			else if (xhr.status === 404) {
				PrintError('Utilisateur introuvable', 'R');
			}
			else {
				PrintError('Erreur de connexion : ' + xhr.status + " - " + errorThrown, 'R');
			}
		},
		complete: function (xhr, textStatus) {
			$('html').removeClass("wait");
