		...
		if (options.needLogin)
		{
			...
			for (var i = 0; i < options.urlParamArray.length; i++) {
				...
				if (!searchParams.has(urlParam)) {
					PrintWarning('Paramètre manquant pour l&apos;autorisation de connexion', 'I');
					break;
				}
			}
...
function onRegistration()
{
	...
	if ((!options.needSaveQuiz) || (options.saveQuizMode !== QMAKE_SEND_NODEJS && options.saveQuizMode !== QMAKE_SEND_NODEJS_LOCAL))
	{
		PrintError('Le quiz n&apos;a pas été créé avec l&apos;option activée: enregistrement sur le serveur', 'X');
		return;
	}
	...
	{
		PrintWarning('Tous les champs avec (*) sont obligatoires', 'X');
	}
	else if (!options.loginInsteadEmail && !ValidateEmail(email))
	{
		PrintWarning('Adresse e-mail incorrecte', 'X');
    }
	else if (pwd !== pwd2)
	{
		PrintWarning('Mot de passe incompatible', 'X');
	}
	else
	{
...
function GetCheckLogin(loginKey, loginData)
{
	...
	$.ajax({
		...
		{
			PrintError('Utilisateur déjà enregistré', 'X');
...
function PostRegistration(loginData)
{
	$.ajax({
		...
		error: function (xhr, textStatus, errorThrown ) {
			PrintError('Erreur d&apos;enregistrement : ' + xhr.status + (errorThrown ? (" - " + errorThrown) : ""), 'X');
		},
		complete: function (xhr, textStatus) {
			$('html').removeClass("wait");
			$('#registrationBtnId').prop("disabled", false);
...
function onLogin()
{
	if (options.needLogin)
	{
		if ((!options.needSaveQuiz) || (options.saveQuizMode !== QMAKE_SEND_NODEJS && options.saveQuizMode !== QMAKE_SEND_NODEJS_LOCAL))
		{
			PrintError('Le quiz n&apos;a pas été créé avec l&apos;option activée: enregistrement sur le serveur', 'I');
			return;
		}
	...
	else
	{
		...
		{
			PrintError('Le nom du champ ne peut pas être vide', 'I');
		}
	}
}
...
function PostLogin(email, passhash, domainId)
{
	...
	$.ajax({
		...
		error: function (xhr, textStatus, errorThrown) {
			...
			if (xhr.status === 401) {
				PrintError('Utilisateur non autorisé', 'I');
			}
			else if (xhr.status === 404) {
				PrintError('Utilisateur introuvable', 'I');
			}
			else  {
				PrintError('Erreur de connexion : ' + xhr.status + (errorThrown ? (' - ' + errorThrown) : ''), 'I');
			}
